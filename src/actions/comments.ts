...
export async function addComment(postId: string, content: string) {
  try {
    ...
    if (childSession) {
      ...
      if (user) {
        // Check if parent first (to show appropriate error)
    ...
    const { data: postForXp } = await supabase
      ...
      .eq("post_id", postId)
      .single();
    await handleCommentStats(postForXp?.child_id, childId, schoolId, true);
...
export async function deleteComment(commentId: string) {
  try {
    // 1. Identify User (Child or School)
    let childId: string | null = null;
    let schoolId: string | null = null;
    ...
    if (!childId && !schoolId) return { success: false, error: "Unauthorized" };

    // 2. Fetch Comment to verify ownership and get post_id
    const { data: comment, error: fetchError } = await supabase
      .from("comments")
      .select("id, post_id, child_id, school_id")
    ...
    // Verify Ownership
    if (childId && comment.child_id !== childId)
      return { success: false, error: "Unauthorized" };
    if (schoolId && comment.school_id !== schoolId)
      return { success: false, error: "Unauthorized" };
    ...
    const { data: postForXp } = await supabase
      ...
      .eq("post_id", comment.post_id)
      .single();
    // Pass the ORIGINAL commenter ID (which is childId or schoolId)
    await handleCommentStats(
      postForXp?.child_id,
      comment.child_id,
      comment.school_id,
      false,
...
async function handleCommentStats(
  postOwnerId: string | null | undefined,
  commenterChildId: string | null | undefined,
  commenterSchoolId: string | null | undefined,
  isAdding: boolean,
) {
  // 1. XP for Post Owner (+5 / -5) - Only for children
  if (postOwnerId) {
    ...
    if (owner) {
      ...
      await supabase
        ...
        .eq("child_id", postOwnerId);
      await invalidateCache(`profile:${postOwnerId}`);
      if (owner.username) {
        await invalidateCache(`profile:${owner.username}`);
      }
    }
  }

  // 2. Total Comments Made for Commenter
  if (commenterChildId) {
    // Child commenter
    const { data: commenter } = await supabase
      .from("children")
      .select("total_comments_made, username")
      .eq("child_id", commenterChildId)
      .single();
    if (commenter) {
      ...
      await supabase
        .from("children")
        .update({ total_comments_made: newTotal })
        .eq("child_id", commenterChildId);
      await invalidateCache(`profile:${commenterChildId}`);
      if (commenter.username) {
        await invalidateCache(`profile:${commenter.username}`);
      }
    }
  } else if (commenterSchoolId) {
    // School commenter
    const { data: school } = await supabase
      .from("schools")
      .select("total_comments_made, school_id")
      .eq("school_id", commenterSchoolId)
      .single();
    if (school) {
      const currentTotal = school.total_comments_made || 0;
      const newTotal = isAdding
        ? currentTotal + 1
        : Math.max(0, currentTotal - 1);
        ...
        .select(
          `
                    *,
                    ...
                    school:schools(name, slug, logo_url)
                `,
        )
        .eq("post_id", postId)
