...
export default function SignUpForm() {
  ...
  useEffect(() => {
    ...
    if (refParam) {
      ...
      localStorage.setItem("qidzo_referral_code", refParam);
      ...
      const storedRef = localStorage.getItem("qidzo_referral_code");
      if (storedRef) {
  ...
  const onProfileSubmit = async (data: ProfileValues) => {
    ...
    try {
      ...
      const updateRes = await updateClerkUserMetadata(
        userId,
        firstName,
        lastName,
        "parent",
      ...
      if (updateRes.success) {
        ...
        console.error(
          "[SignUp] Failed to update Clerk Metadata:",
          updateRes.error,
        );
      ...
      if (referralCode) {
        console.log("[SignUp] Applying referral code:", referralCode);
        const referralResult = await applyReferralCode(referralCode, parentId);
        ...
        if (referralResult.success) {
          ...
          localStorage.removeItem("qidzo_referral_code");
        } else {
          console.error(
            "[SignUp] Referral application failed:",
            referralResult.error,
          );
          // Don't block signup if referral fails, just log it
          toast.info("Note: Referral code could not be applied", {
            description: referralResult.error || "Invalid or expired code",
            duration: 4000,
          });
        }
      }
    ...
    <div className="w-full max-w-md mx-auto bg-white rounded-3xl shadow-xl overflow-hidden border-4 border-gray-100">
      ...
      <div className="p-8">
        <div className="text-center mb-8">
          ...
          {referralCode && step === "auth" && (
            <div className="mt-3 inline-flex items-center gap-2 px-4 py-2 bg-grass-green/10 border-2 border-grass-green/20 rounded-xl">
              <Gift className="w-4 h-4 text-grass-green" />
              <span className="text-sm font-bold text-grass-green">
                      ...
                      <>
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          {Array.from({ length: 3 }).map((_, i) => {
                            const base = field.value.replace(
                              /[^a-zA-Z0-9_]/g,
                              "",
                            );
                            const suggestion =
                            ...
                            return (
                              <button
                                ...
                                onClick={() => {
                                  profileForm.setValue("username", suggestion, {
                                    shouldValidate: true,
                                  });
