  ...
  return (
    <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 h-full">
      ...
      <div className="space-y-4">
        {MOCK_POSTS.map((post) => (
          <div key={post.id} className="flex gap-4 p-3 rounded-2xl hover:bg-gray-50 transition-colors cursor-pointer group">
            ...
            <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start">
                    <div>
                        <span className="text-[10px] font-black uppercase tracking-wider text-brand-purple bg-brand-purple/10 px-2 py-0.5 rounded-full">{post.category}</span>
                        <h4 className="font-bold text-gray-900 mt-1 truncate">{post.caption}</h4>
                    </div>
                    <span className="text-xs font-bold text-gray-400 whitespace-nowrap">{post.timestamp}</span>
                </div>
